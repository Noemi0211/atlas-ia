...
- **Despliegue de producción COMPLETADO el 14/9/2026.** Repo GitHub público `Noemi0211/atlas-ia` (rama `master`), proyecto Vercel `atlas-ia` enlazado, app operativa en **https://atlas-ia-psi.vercel.app** (el dominio `atlas-ia.vercel.app` está ocupado por una agencia chilena; `atlas-ia-noemi0211.vercel.app` responde "Login - Vercel" por `ssoProtection.all_except_custom_domains` — la URL pública válida es la `psi`). Env vars de Vercel (production): `DATABASE_URL` pooled + `DIRECT_URL` directa (Neon), `NEXTAUTH_SECRET` nuevo 64-hex, `NEXTAUTH_URL=https://atlas-ia-psi.vercel.app`, `TEACHER_EMAILS=profecelaya@gmail.com`. Build de Vercel: `prisma generate && next build` (commit `b4b5a81`) porque Vercel cachea dependencias y no auto-genera Prisma Client. **`SITE_CONFIG.url` y metadatos SEO apuntan a `https://atlas-ia-psi.vercel.app`** (commit `1d47051`; canonical/og:url verificados en producción; comentario de ejemplo del asistente en `lib/ai-knowledge.ts` actualizado). Decisiones de dominio: la autora eligió (14/9/2026) usar `vercel.app` de momento; revivir `atlas-ia.dev` cuando se configure (SITE_CONFIG.url + NEXTAUTH_URL + Vercel domains).
- **Cuenta docente de la autora creada y verificada en producción** (registro HTTP 201, login NextAuth 302, `role: teacher`, `/docencia` + `/api/docencia/students` 200). Correo: `profecelaya@gmail.com` (en `TEACHER_EMAILS` de Vercel y `.env` local). Datos de prueba de la e2e eliminados (BD limpia, solo esta cuenta).
- **Prueba e2e en producción completada (14/9/2026)**: registro/login estudiante → feedback POST 200 → propuesta POST 201 → sync-progress POST 200 (XP 120) → vista docente OK (media 5, `ratings[]`, propuesta con usuario, PATCH `pending`→`revisada` 200) → protección de rutas (307 → login sin sesión; estudiante redirigido). Nota: el PATCH con `--data-raw '{...}'` da 500 por el bug de comillas de PowerShell 5.1 con programas nativos (no es bug del servidor); usar `--data-binary "@archivo.json"` (mismo patrón Fase 33/Fase 57).
- Fases 43–56 completadas y commiteadas; **Fase 57 commit `4f530c3`** (SQLite → PostgreSQL/Neon, ver sección "Fase 57"; `.env` local gitignored con URLs reales de Neon).
...
- Siguientes pasos: ver **"Plan de trabajo pendiente"** (sección tras la Fase 57): despliegue de la prueba piloto (redactar el correo de invitación para 6 personas evaluadoras y la hoja de seguimiento).
...
1. **✅ Commitear la Fase 57** — commit `4f530c3` + push a `Noemi0211/atlas-ia` (público). `.env` gitignored.
2. **✅ Migración a Postgres** — aplicada en Neon (13/9/2026); `DATABASE_URL` (pooled) y `DIRECT_URL` ya configuradas en Vercel.
3. **✅ Variables de entorno en Vercel (production)** — DATABASE_URL/DIRECT_URL reales de Neon, NEXTAUTH_SECRET nuevo 64-hex, `NEXTAUTH_URL=https://atlas-ia-psi.vercel.app`, `TEACHER_EMAILS=profecelaya@gmail.com`. Falta solo `SENTRY_DSN` real (recomendado, no bloqueante).
4. **✅ Cuenta docente de la autora** — `profecelaya@gmail.com` / rol `teacher` verificada en producción (14/9/2026): registro 201, login NextAuth 302, `/docencia` + `/api/docencia/students` 200.
5. **✅ Prueba e2e en producción (14/9/2026)** — estudiante de prueba: registro 201, login, feedback POST 200, suggestions POST 201, sync-progress 200 (XP 120), vista docente (media 5, ratings[], propuesta pending→revisada PATCH 200), protección de rutas (307 → login sin sesión; estudiante redirigido). Datos de prueba eliminados (BD limpia); solo queda la cuenta docente.
