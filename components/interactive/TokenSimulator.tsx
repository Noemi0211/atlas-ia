...
export function TokenSimulator() {
  ...
  const thresholds = [
    { limit: 0.75, color: "text-success", bg: "bg-success/10", border: "border-success/20", label: t.lab.tokenSimulator.withinLimit },
    { limit: 0.9, color: "text-warning", bg: "bg-warning/10", border: "border-warning/20", label: t.lab.tokenSimulator.nearLimit },
    { limit: 1.0, color: "text-error", bg: "bg-error/10", border: "border-error/20", label: t.lab.tokenSimulator.limitReached },
  ];
  ...
  const threshold = thresholds.find((th) => usage <= th.limit) || thresholds[thresholds.length - 1];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold text-fg">{t.lab.tokenSimulator.title}</h3>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-fg-secondary mb-1.5">{t.lab.tokenSimulator.inputLabel}</label>
            <textarea
              value={text}
              onChange={(e) => setText(e.target.value)}
              placeholder={t.lab.tokenSimulator.inputPlaceholder}
              rows={10}
          ...
          <div className="flex flex-wrap gap-1.5">
            <span className="text-2xs text-fg-muted self-center">{t.lab.tokenSimulator.examples}</span>
            {EXAMPLES.map((ex, i) => (
              <button
                key={i}
              ...
              >
                {t.lab.tokenSimulator.example} {i + 1}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-fg-secondary mb-1.5">{t.lab.tokenSimulator.model}</label>
            <select
          ...
          <div className={`p-4 rounded-lg border ${threshold.border} ${threshold.bg}`}>
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-medium text-fg">{t.lab.tokenSimulator.estimatedTokens}</span>
            ...
            <div className="mt-3 space-y-1">
              <div className="flex justify-between text-xs">
                <span className="text-fg-secondary">{t.lab.tokenSimulator.totalContext}</span>
                <span className="text-fg">{selectedModel.context.toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-xs">
                <span className="text-fg-secondary">{t.lab.tokenSimulator.usedTokens}</span>
                <span className={threshold.color}>{tokens.toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-xs">
                <span className="text-fg-secondary">{t.lab.tokenSimulator.remainingTokens}</span>
                <span className="text-fg">{remaining.toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-xs">
                <span className="text-fg-secondary">{t.lab.tokenSimulator.usage}</span>
            ...
            <div className={`flex items-center gap-1.5 mt-3 text-2xs ${threshold.color}`}>
              ...
              {usage > 0.9
                ? t.lab.tokenSimulator.overLimit
                : threshold.label}
            </div>
          </div>

          <div className="p-3 rounded-lg bg-bg-secondary border border-border">
            <h4 className="text-2xs font-medium text-fg-secondary mb-1">{t.lab.tokenSimulator.howCalculated}</h4>
            <p className="text-2xs text-fg-muted leading-relaxed">
              {t.lab.tokenSimulator.calcHint1}{" "}
              {t.lab.tokenSimulator.calcHint2}
            </p>
          </div>
        </div>
      </div>
